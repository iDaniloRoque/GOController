...
function scJQEventsAdd(iSeqRow) {
  ...
  $('#id_sc_field_idfilial' + iSeqRow).bind('blur', function() { sc_sec_form_edit_users_idfilial_onblur(this, iSeqRow) })
                                      .bind('focus', function() { sc_sec_form_edit_users_idfilial_onfocus(this, iSeqRow) });
...
function sc_sec_form_edit_users_idfilial_onblur(oThis, iSeqRow) {
  do_ajax_sec_form_edit_users_mob_validate_idfilial();
  scCssBlur(oThis);
}

function sc_sec_form_edit_users_idfilial_onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis);
}
...
function displayChange_field_idfilial(row, status) {
	if ("on" == status) {
		if ("all" == row) {
			var fieldList = $(".css_idfilial__obj");
			for (var i = 0; i < fieldList.length; i++) {
				$($(fieldList[i]).attr("id")).select2("destroy");
			}
		}
		else {
			$("#id_sc_field_idfilial" + row).select2("destroy");
		}
		scJQSelect2Add(row, "idfilial");
	}
}

function scRecreateSelect2() {
	displayChange_field_idfilial("all", "on");
}
...
function scJQSelect2Add(seqRow, specificField) {
  if (null == specificField || "idfilial" == specificField) {
    scJQSelect2Add_idfilial(seqRow);
  }
} // scJQSelect2Add

function scJQSelect2Add_idfilial(seqRow) {
  var elemSelector = "all" == seqRow ? ".css_idfilial_obj" : "#id_sc_field_idfilial" + seqRow;
  $(elemSelector).select2(
    {
      containerCssClass: 'css_idfilial_obj',
      dropdownCssClass: 'css_idfilial_obj',
      language: {
        noResults: function() {
...
function scJQElementsAdd(iLine) {
  ...
  setTimeout(function () { if ('function' == typeof displayChange_field_idfilial) { displayChange_field_idfilial(iLine, "on"); } }, 150);
} // scJQElementsAdd
