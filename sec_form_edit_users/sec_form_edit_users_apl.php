...
//
      ...
      if ($this->nmgp_opcao == "recarga" || $this->nmgp_opcao == "recarga_mobile" || $this->nmgp_opcao == "muda_form") 
      {
          ...
          {
              $this->idfilial = explode("@?@", $this->idfilial);
          }
          if (is_array($this->idfilial))
          {
              $x = 0; 
              $this->idfilial_1 = $this->idfilial;
              $this->idfilial = ""; 
              if ($this->idfilial_1 != "") 
              { 
                  foreach ($this->idfilial_1 as $dados_idfilial_1 ) 
                  { 
                      if ($x != 0)
                      { 
                          $this->idfilial .= ",";
                      } 
                      $this->idfilial .= $dados_idfilial_1;
                      $x++ ; 
                  } 
              } 
          } 
...
//---------------------------------------------------------
    ...
    function ValidateField_idfilial(&$Campos_Crit, &$Campos_Falta, &$Campos_Erros)
    {
      ...
      if (!empty($this->idfilial))
      {
          ...
          $idfilial_SC    = explode(',', $this->idfilial);
          foreach ($idfilial_SC as $cada_cmp_SC)
          {
              if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['Lookup_idfilial']) && !in_array($cada_cmp_SC, $_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['Lookup_idfilial']))
              {
                  $hasError = true;
                  $Campos_Crit .= $this->Ini->Nm_lang['lang_errm_ajax_data'];
...
$nmgp_def_dados = "" ; 
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['Lookup_idfilial']))
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['Lookup_idfilial'] = array_unique($_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['Lookup_idfilial']); 
}
else
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['Lookup_idfilial'] = array(); 
}
   if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_ibase))
   { 
       $GLOBALS["NM_ERRO_IBASE"] = 1;  
   ...
   if ($nm_comando != "" && $rs = $this->Db->Execute($nm_comando))
   {
       ...
       { 
              ...
              $nmgp_def_dados .= $rs->fields[1] . "?#?" ; 
              $nmgp_def_dados .= $rs->fields[0] . "?#?N?@?" ; 
              $_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['Lookup_idfilial'][] = $rs->fields[0];
              $rs->MoveNext() ; 
       } 
       $rs->Close() ; 
   } 
