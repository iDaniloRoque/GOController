...
//---------------------------------------------------------
    ...
    function ValidateField_idfilial(&$Campos_Crit, &$Campos_Falta, &$Campos_Erros)
    {
        global $teste_validade;
        $hasError = false;
               if (!empty($this->idfilial) && isset($_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users_mob']['Lookup_idfilial']) && !in_array($this->idfilial, $_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users_mob']['Lookup_idfilial']))
               {
                   $hasError = true;
                   $Campos_Crit .= $this->Ini->Nm_lang['lang_errm_ajax_data'];
...
else
{
   ...
   {
       if (is_array($this->idfilial))
       {
           $Tmp_array = $this->idfilial;
       }
       else
       {
           $Tmp_array = explode(",", $this->idfilial);
       }
       $idfilial_val_str = "";
       foreach ($Tmp_array as $Tmp_val_cmp)
       {
   ...
   while (!empty($todo[$x])) 
   {
          ...
          $tempOptionId = "id-opt-idfilial-" . $x;
          echo "      <input type=checkbox id=\"" . $tempOptionId . "\" class=\"sc-ui-checkbox-groups sc-ui-checkbox-idfilial\" name=\"idfilial[$ind]\" value=\"$cadacheck[1]\"" ; 
...
$nmgp_def_dados = "" ; 
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users_mob']['Lookup_idfilial']))
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users_mob']['Lookup_idfilial'] = array_unique($_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users_mob']['Lookup_idfilial']); 
}
else
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users_mob']['Lookup_idfilial'] = array(); 
}
   if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_ibase))
   { 
       $GLOBALS["NM_ERRO_IBASE"] = 1;  
   ...
   if ($nm_comando != "" && $rs = $this->Db->Execute($nm_comando))
   {
       ...
       { 
              ...
              $nmgp_def_dados .= $rs->fields[1] . "?#?" ; 
              $nmgp_def_dados .= $rs->fields[0] . "?#?N?@?" ; 
              $_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users_mob']['Lookup_idfilial'][] = $rs->fields[0];
              $rs->MoveNext() ; 
       } 
       $rs->Close() ; 
   } 
   ...
   $GLOBALS["NM_ERRO_IBASE"] = 0; 
          ...
          {
              $this->NM_ajax_info['fldList']['idfilial']['valList'][$i] = sec_form_edit_users_mob_pack_protect_string($v);
          }
          foreach ($aLookupOrig as $aValData)
          {
...
if (isset($this->NM_ajax_flag) && $this->NM_ajax_flag)
{
    $original_login = $this->login;
      ...
      {
          $sc_cmd_dependency = "DELETE FROM dbo.sc_users_groups WHERE login = '" . $this->login  . "'";
      }
...
$_SESSION['scriptcase']['sec_form_edit_users_mob']['contr_erro'] = 'off'; 
      ...
      if ($this->nmgp_opcao == "alterar" || $this->nmgp_opcao == "incluir") 
      {
          ...
          $this->idfilial_before_qstr = $this->idfilial;
          $this->idfilial = substr($this->Db->qstr($this->idfilial), 1, -1); 
          if ($this->idfilial == "" && in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))  
          { 
              $this->idfilial = "null"; 
              $NM_val_null[] = "idfilial";
          } 
      }
      if ($this->nmgp_opcao == "alterar") 
      {
          ...
          { 
              ...
              { 
                  ...
                  $SC_fields_update[] = "name = '$this->name', email = '$this->email', active = '$this->active', idFilial = '$this->idfilial'"; 
              } 
              elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
              { 
                  $comando = "UPDATE " . $this->Ini->nm_tabela . " SET ";  
             ...
             { 
              ...
              $this->idfilial = $this->idfilial_before_qstr;
              ...
              $this->nmgp_refresh_fields = array_diff(array('login', 'pswd', 'confirm_pswd', 'name', 'email', 'active', 'groups', 'idfilial'), $aDoNotUpdate);
              $this->ajax_return_values();
          ...
          { 
              $_SESSION['scriptcase']['sc_sql_ult_comando'] = "select login, group_id from dbo.sc_users_groups where login = '$this->login'"; 
          }  
          elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
          { 
              $_SESSION['scriptcase']['sc_sql_ult_comando'] = "select login, group_id from dbo.sc_users_groups where login = '$this->login'"; 
         ...
         { 
              ...
              { 
                  $_SESSION['scriptcase']['sc_sql_ult_comando'] = "delete from dbo.sc_users_groups where login = '" . $sc_campos_sel['login'] . "' and group_id = '" . $sc_campos_sel['group_id'] . "'"; 
              } 
              else 
              { 
                  $_SESSION['scriptcase']['sc_sql_ult_comando'] = "delete from dbo.sc_users_groups where login = '" . $sc_campos_sel['login'] . "' and group_id = '" . $sc_campos_sel['group_id'] . "'"; 
         ...
         { 
             if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
             { 
                 $_SESSION['scriptcase']['sc_sql_ult_comando'] = "insert into dbo.sc_users_groups (login, group_id) values ('" . $sc_campos_form['login'] . "', '" . $sc_campos_form['groups'] . "')"; 
             } 
             else 
             { 
                 $_SESSION['scriptcase']['sc_sql_ult_comando'] = "insert into dbo.sc_users_groups (login, group_id) values ('" . $sc_campos_form['login'] . "', '" . $sc_campos_form['groups'] . "')"; 
      ...
      { 
          ...
          { 
              if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
              { 
                  $comando = "INSERT INTO " . $this->Ini->nm_tabela . " (login, pswd, name, email, active, activation_code, priv_admin, idFilial) VALUES ('$this->login', '$this->pswd', '$this->name', '$this->email', '$this->active', '$this->activation_code', '$this->priv_admin', '$this->idfilial')"; 
              }
              elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
              { 
                  $comando = "INSERT INTO " . $this->Ini->nm_tabela . " (" . $NM_cmp_auto . "login, pswd, name, email, active, activation_code, priv_admin, idFilial) VALUES (" . $NM_seq_auto . "'$this->login', '$this->pswd', '$this->name', '$this->email', '$this->active', '$this->activation_code', '$this->priv_admin', '$this->idfilial')"; 
              }
          ...
          { 
              foreach ($todo_groups as $cada_groups) 
              {
                  ...
                  { 
                      ...
                      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
                      { 
                          $_SESSION['scriptcase']['sc_sql_ult_comando'] = "insert into dbo.sc_users_groups (login, group_id) values ('$this->login', '$groupsx')"; 
                      } 
                      else 
                      { 
                          $_SESSION['scriptcase']['sc_sql_ult_comando'] = "insert into dbo.sc_users_groups (login, group_id) values ('$this->login', '$groupsx')"; 
      ...
      { 
          ...
          { 
              ...
              if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
              { 
                  $_SESSION['scriptcase']['sc_sql_ult_comando'] = "delete from dbo.sc_users_groups where login = '$this->login'"; 
              } 
              else 
              { 
                  $_SESSION['scriptcase']['sc_sql_ult_comando'] = "delete from dbo.sc_users_groups where login = '$this->login'"; 
...
//---------- 
      ...
      { 
          ...
          if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
          { 
              $nmgp_select = "SELECT login, pswd, name, email, active, activation_code, priv_admin, idFilial from " . $this->Ini->nm_tabela ; 
          } 
          elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
          { 
              $nmgp_select = "SELECT login, pswd, name, email, active, activation_code, priv_admin, idFilial from " . $this->Ini->nm_tabela ; 
    ...
    function getButtonIds($buttonName) {
        switch ($buttonName) {
            ...
            case "exit":
                ...
                break;
            case "new":
                return array("sc_b_new_b.sc-unique-btn-4", "sc_b_new_b.sc-unique-btn-11");
                break;
            case "insert":
                return array("sc_b_ins_b.sc-unique-btn-5", "sc_b_ins_b.sc-unique-btn-12");
                break;
            case "update":
                return array("sc_b_upd_b.sc-unique-btn-6", "sc_b_upd_b.sc-unique-btn-13");
                break;
            case "delete":
                return array("sc_b_del_b.sc-unique-btn-7", "sc_b_del_b.sc-unique-btn-14");
                break;
        }
