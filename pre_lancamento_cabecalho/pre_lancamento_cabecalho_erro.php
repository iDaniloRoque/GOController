...
$old_error_handler = set_error_handler("pre_lancamento_cabecalho_scriptcase_error_handler");

function pre_lancamento_cabecalho_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['pre_lancamento_cabecalho']['contr_erro']) && $_SESSION['scriptcase']['pre_lancamento_cabecalho']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_pre_lancamento_cabecalho->contr_pre_lancamento_cabecalho->Apl_com_erro = true;
            if ($inicial_pre_lancamento_cabecalho->contr_pre_lancamento_cabecalho->NM_ajax_flag)
            {
                $inicial_pre_lancamento_cabecalho->contr_pre_lancamento_cabecalho->NM_ajax_info['errList']['geral_pre_lancamento_cabecalho'][] = $msg;
                return;
            }
            pre_lancamento_cabecalho_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function pre_lancamento_cabecalho_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['pre_lancamento_cabecalho'])  && '' != $_SESSION['scriptcase']['error_icon']['pre_lancamento_cabecalho'])  ? $_SESSION['scriptcase']['error_icon']['pre_lancamento_cabecalho']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['pre_lancamento_cabecalho']) && '' != $_SESSION['scriptcase']['error_close']['pre_lancamento_cabecalho']) ? $_SESSION['scriptcase']['error_close']['pre_lancamento_cabecalho'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['pre_lancamento_cabecalho']) && '' != $_SESSION['scriptcase']['error_span_title']['pre_lancamento_cabecalho']) ? $_SESSION['scriptcase']['error_span_title']['pre_lancamento_cabecalho'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['pre_lancamento_cabecalho']) && '' != $_SESSION['scriptcase']['error_icon_title']['pre_lancamento_cabecalho']) ? $_SESSION['scriptcase']['error_icon_title']['pre_lancamento_cabecalho'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['pre_lancamento_cabecalho']['error_buffer'] .= ob_get_flush();
}
