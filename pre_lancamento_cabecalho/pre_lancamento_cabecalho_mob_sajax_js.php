  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_pre_lancamento_cabecalho_mob" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_pre_lancamento_cabecalho_mob_form();
	}
...
?>
  ...
  // ---------- Validate data
  function do_ajax_pre_lancamento_cabecalho_mob_validate_data()
  {
    var nomeCampo_data = "data";
    var var_data = scAjaxGetFieldText(nomeCampo_data);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_pre_lancamento_cabecalho_mob_validate_data(var_data, var_script_case_init, do_ajax_pre_lancamento_cabecalho_mob_validate_data_cb);
  } // do_ajax_pre_lancamento_cabecalho_mob_validate_data

  function do_ajax_pre_lancamento_cabecalho_mob_validate_data_cb(sResp)
  {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_pre_lancamento_cabecalho_mob_validate_data_cb

  // ---------- Validate especialidade
  function do_ajax_pre_lancamento_cabecalho_mob_validate_especialidade()
  {
    var nomeCampo_especialidade = "especialidade";
    var var_especialidade = scAjaxGetFieldSelect(nomeCampo_especialidade);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_pre_lancamento_cabecalho_mob_validate_especialidade(var_especialidade, var_script_case_init, do_ajax_pre_lancamento_cabecalho_mob_validate_especialidade_cb);
  } // do_ajax_pre_lancamento_cabecalho_mob_validate_especialidade

  function do_ajax_pre_lancamento_cabecalho_mob_validate_especialidade_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_pre_lancamento_cabecalho_mob_validate_especialidade_cb
...
function scJs_sweetalert_params(params) {
  ...
  // ---------- Form
  function do_ajax_pre_lancamento_cabecalho_mob_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_pre_lancamento_cabecalho_mob_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    ...
    scAjaxProcOn();
    x_ajax_pre_lancamento_cabecalho_mob_submit_form(var_data, var_especialidade, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_pre_lancamento_cabecalho_mob_submit_form_cb);
  } // do_ajax_pre_lancamento_cabecalho_mob_submit_form

  function do_ajax_pre_lancamento_cabecalho_mob_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
    ...
    scAjaxSetLabel();
    scAjaxSetReadonly();
    scAjaxAlert(do_ajax_pre_lancamento_cabecalho_mob_submit_form_cb_after_alert);
  } // do_ajax_pre_lancamento_cabecalho_mob_submit_form_cb
  function do_ajax_pre_lancamento_cabecalho_mob_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_pre_lancamento_cabecalho_mob_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_pre_lancamento_cabecalho_mob_navigate_form()
  {
    if (scFormHasChanged) {
      scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', perform_ajax_pre_lancamento_cabecalho_mob_navigate_form, function() {});
    } else {
      perform_ajax_pre_lancamento_cabecalho_mob_navigate_form();
    }
  }

  function perform_ajax_pre_lancamento_cabecalho_mob_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    }
  } // do_ajax_pre_lancamento_cabecalho_mob_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['pre_lancamento_cabecalho_mob']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_pre_lancamento_cabecalho_mob_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetBtnVars();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_pre_lancamento_cabecalho_mob_navigate_form_cb_after_alert);
  } // do_ajax_pre_lancamento_cabecalho_mob_navigate_form_cb
  function do_ajax_pre_lancamento_cabecalho_mob_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
<?php
...
{
?>
    do_ajax_pre_lancamento_cabecalho_mob_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_pre_lancamento_cabecalho_mob_navigate_form_cb_after_alert
  function sc_hide_pre_lancamento_cabecalho_mob_form()
  {
  ...
  var ajax_error_list = {
    "data": {"label": "Mes", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5},
