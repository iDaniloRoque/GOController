...
class grid_dbo_view_contratos_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_dbo_view_contratos']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_dbo_view_contratos']['campos_busca']))
      { 
   ...
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_contratos']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_contratos']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_contratos']['where_pesq']; 
      } 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_contratos']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_contratos']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_contratos']['contr_total_geral'] = "OK";
   } 

   //-----  comercial
   function quebra_comercial_sc_free_group_by($comercial, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_comercial = array() ;  
      $tot_comercial[0] = $comercial ; 
   }
   //-----  nome_razao_social
   function quebra_nome_razao_social_sc_free_group_by($nome_razao_social, $Where_qb, $Cmp_Name) 
   {
      ...
      $tot_nome_razao_social[0] = $nome_razao_social ; 
   }
   //-----  situacao
   function quebra_situacao_sc_free_group_by($situacao, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_situacao = array() ;  
      $tot_situacao[0] = $situacao ; 
   }
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("pt_br");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_contratos']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_contratos']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('comercial' => "comercial",'nome_razao_social' => "nome_razao_social",'situacao' => "situacao");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_contratos']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_contratos']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_contratos']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      $cmp_sql_tp_num = array('idfilial' => 'N','idpessoas' => 'N','valor' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_contratos']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_contratos']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_contratos']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_contratos']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['comercial']['reg'] = "S";
      $format_dimensions['comercial']['msk'] = "";
      $format_dimensions['nome_razao_social']['reg'] = "S";
      $format_dimensions['nome_razao_social']['msk'] = "";
      $format_dimensions['situacao']['reg'] = "S";
      $format_dimensions['situacao']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "comercial") {
              }
              if ($Cada_dim == "nome_razao_social") {
              }
              if ($Cada_dim == "situacao") {
                  $this->Lookup->lookup_sc_free_group_by_situacao($$Cada_dim); 
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
