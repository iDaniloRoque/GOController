...
class grid_dbo_view_conferencia_guia_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_dbo_view_conferencia_guia']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_dbo_view_conferencia_guia']['campos_busca']))
      { 
   ...
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_conferencia_guia']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_conferencia_guia']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(total_servico) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_conferencia_guia']['where_pesq']; 
      } 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_conferencia_guia']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_conferencia_guia']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_conferencia_guia']['tot_geral'][2] = $rt->fields[1]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_conferencia_guia']['contr_total_geral'] = "OK";
   } 

   //-----  nome_razao_social
   function quebra_nome_razao_social_sc_free_group_by($nome_razao_social, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_nome_razao_social = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(total_servico) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_conferencia_guia']['where_pesq']; 
      } 
      ...
      $tot_nome_razao_social[0] = sc_strip_script($nome_razao_social) ; 
      $tot_nome_razao_social[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_nome_razao_social[2] = (string)$rt->fields[1]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_nome_razao_social;
   } 

   //-----  contratos
   function quebra_contratos_sc_free_group_by($contratos, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_contratos = array() ;  
      ...
      $tot_contratos[0] = sc_strip_script($contratos) ; 
      $tot_contratos[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_contratos[2] = (string)$rt->fields[1]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_contratos;
   } 

   //-----  profissional
   function quebra_profissional_sc_free_group_by($profissional, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_profissional = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(total_servico) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_conferencia_guia']['where_pesq']; 
      } 
      ...
      $tot_profissional[0] = sc_strip_script($profissional) ; 
      $tot_profissional[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_profissional[2] = (string)$rt->fields[1]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_profissional;
   } 

   //-----  descricao_especialidade
   function quebra_descricao_especialidade_sc_free_group_by($descricao_especialidade, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_descricao_especialidade = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(total_servico) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_conferencia_guia']['where_pesq']; 
      } 
      ...
      $tot_descricao_especialidade[0] = sc_strip_script($descricao_especialidade) ; 
      $tot_descricao_especialidade[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_descricao_especialidade[2] = (string)$rt->fields[1]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_descricao_especialidade;
   } 

   //-----  descricao_servico
   function quebra_descricao_servico_sc_free_group_by($descricao_servico, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_descricao_servico = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(total_servico) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_conferencia_guia']['where_pesq']; 
      } 
      ...
      $tot_descricao_servico[0] = sc_strip_script($descricao_servico) ; 
      $tot_descricao_servico[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_descricao_servico[2] = (string)$rt->fields[1]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_descricao_servico;
   } 
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("pt_br");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_conferencia_guia']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_conferencia_guia']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('nome_razao_social' => "nome_razao_social",'contratos' => "Contratos",'profissional' => "Profissional",'descricao_especialidade' => "descricao_especialidade",'descricao_servico' => "descricao_servico");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_conferencia_guia']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_conferencia_guia']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_conferencia_guia']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      $cmp_sql_tp_num = array('vlr_servico' => 'N','quantidade' => 'N','numero_da_guia' => 'N','idpessoas' => 'N','total_servico' => 'N','idfilial' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), sum(total_servico)#@#cmps_quebras#@# from " . $this->Ini->nm_tabela . " " . $where_ok;
         $comando  = "select count(*), sum(SC_metric1)#@#cmps_quebras#@# from (";
         $comando .= "select total_servico as SC_metric1, " . $cmps_quebras1 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . ") SC_sel1 INNER JOIN (";
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_conferencia_guia']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_conferencia_guia']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_conferencia_guia']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_view_conferencia_guia']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['nome_razao_social']['reg'] = "S";
      $format_dimensions['nome_razao_social']['msk'] = "";
      $format_dimensions['contratos']['reg'] = "S";
      $format_dimensions['contratos']['msk'] = "";
      $format_dimensions['profissional']['reg'] = "S";
      $format_dimensions['profissional']['msk'] = "";
      $format_dimensions['descricao_especialidade']['reg'] = "S";
      $format_dimensions['descricao_especialidade']['msk'] = "";
      $format_dimensions['descricao_servico']['reg'] = "S";
      $format_dimensions['descricao_servico']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "nome_razao_social") {
              }
              if ($Cada_dim == "contratos") {
              }
              if ($Cada_dim == "profissional") {
              }
              if ($Cada_dim == "descricao_especialidade") {
              }
              if ($Cada_dim == "descricao_servico") {
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
