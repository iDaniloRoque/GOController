...
function scJQEventsAdd(iSeqRow) {
  ...
  $('#id_sc_field_momento_hora' + iSeqRow).bind('blur', function() { sc_form_dbo_pessoa_juridica_fisica_momento_onblur(this, iSeqRow) })
                                          .bind('focus', function() { sc_form_dbo_pessoa_juridica_fisica_momento_onfocus(this, iSeqRow) });
...
function displayChange_field_socios(row, status) {
	if ("on" == status && typeof $("#nmsc_iframe_liga_form_dbo_pessoa_juridica_fisica_socios_mob")[0].contentWindow.scRecreateSelect2 === "function") {
		$("#nmsc_iframe_liga_form_dbo_pessoa_juridica_fisica_socios_mob")[0].contentWindow.scRecreateSelect2();
	}
}

function displayChange_field_impostos(row, status) {
	if ("on" == status && typeof $("#nmsc_iframe_liga_form_dbo_pessoa_juridica_fisica_impostos_mob")[0].contentWindow.scRecreateSelect2 === "function") {
		$("#nmsc_iframe_liga_form_dbo_pessoa_juridica_fisica_impostos_mob")[0].contentWindow.scRecreateSelect2();
	}
}
...
function scHidePage(pageNo) {
	$("#id_form_dbo_pessoa_juridica_fisica_mob_form" + pageNo).hide();
}
...
function scJQCalendarAdd(iSeqRow) {
  $("#id_sc_field_momento" + iSeqRow).datepicker({
    ...
    onClose: function(dateText, inst) {
      do_ajax_form_dbo_pessoa_juridica_fisica_mob_validate_momento(iSeqRow);
    },
    showWeek: true,
    numberOfMonths: 1,
    changeMonth: true,
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_dbo_pessoa_juridica_fisica_mob')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
