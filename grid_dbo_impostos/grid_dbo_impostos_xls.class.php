...
class grid_dbo_impostos_xls
{
   ...
   function monta_xls()
   {
      ...
      if (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_impostos']['embutida']) {
          ...
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_impostos']['opcao'] = "";
      }
   }
   ...
   function inicializa_vars()
   {
      ...
      { 
          ...
          if (!is_file($this->Ini->root . $this->Ini->path_link . "grid_dbo_impostos/grid_dbo_impostos_res_xls.class.php"))
          {
              $this->Tem_xls_res  = false;
          }
          if ($this->Tem_xls_res)
          { 
              ...
              $this->Res_xls = new grid_dbo_impostos_res_xls();
              $this->prep_modulos("Res_xls");
          } 
          $this->Arquivo    = "sc_xls";
          $this->Arquivo   .= "_" . date("YmdHis") . "_" . rand(0, 1000);
          $this->Arq_zip    = $this->Arquivo . "_grid_dbo_impostos.zip";
          $this->Arquivo   .= "_grid_dbo_impostos" . $this->Xls_tp;
          $this->Tit_doc    = "grid_dbo_impostos" . $this->Xls_tp;
          $this->Tit_zip    = "grid_dbo_impostos.zip";
      ...
      if (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_impostos']['embutida'] && !$this->Ini->sc_export_ajax) {
          ...
          $this->pb->setReturnOption($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_impostos']['xls_return']);
          if ($this->Tem_xls_res) {
   ...
   function grava_arquivo()
   {
      ...
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_impostos']['field_order'] as $Cada_cmp)
      {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_impostos']['xls_name']))
      {
          $Pos = strrpos($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_impostos']['xls_name'], ".");
          if ($Pos === false) {
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_impostos']['xls_name'] .= $this->Xls_tp;
          }
          $this->Arquivo = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_impostos']['xls_name'];
          $this->Arq_zip = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_impostos']['xls_name'];
          $this->Tit_doc = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_impostos']['xls_name'];
          $Pos = strrpos($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_impostos']['xls_name'], ".");
          if ($Pos !== false) {
              $this->Arq_zip = substr($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_impostos']['xls_name'], 0, $Pos);
          }
          $this->Arq_zip .= ".zip";
          $this->Tit_zip  = $this->Arq_zip;
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_impostos']['xls_name']);
          $this->Xls_f = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo;
          $this->Zip_f = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arq_zip;
      }
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_impostos']['embutida_label']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_impostos']['embutida_label'])
      { 
          $this->count_span = 0;
          $this->Xls_row++;
          $this->proc_label();
     ...
     $nm_houve_quebra = "N";
         ...
         if (isset($this->NM_Row_din) && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_impostos']['embutida'])
         { 
      ...
      $this->xls_set_style();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_impostos']['embutida'] && $prim_reg)
      { 
          $this->proc_label();
          $this->xls_sub_cons_copy_label($this->Xls_row);
      ...
      if (isset($this->NM_Col_din) && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_impostos']['embutida'])
      { 
      ...
      { 
          if ($this->Tem_xls_res)
          { 
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_impostos']['xls_res_grid'] = true;
              if (!$this->Ini->sc_export_ajax) {
              ...
              if ($this->Use_phpspreadsheet) {
                  $Xls_res = \PhpOffice\PhpSpreadsheet\IOFactory::load($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_impostos']['xls_res_sheet']);
              }
              else {
                  $Xls_res = PHPExcel_IOFactory::load($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_impostos']['xls_res_sheet']);
              }
              foreach($Xls_res->getAllSheets() as $sheet)
              {
                  $this->Xls_dados->addExternalSheet($sheet);
              }
              unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_impostos']['xls_res_grid']);
              unlink($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_impostos']['xls_res_sheet']);
          } 
          if (!$this->Ini->sc_export_ajax) {
   ...
   function NM_export_idimpostos()
   {
         ...
         if (is_numeric($this->idimpostos))
         {
             $this->NM_ctrl_style[$current_cell_ref]['format'] = '#,##0';
         }
         $this->Nm_ActiveSheet->setCellValue($current_cell_ref . $this->Xls_row, $this->idimpostos);
         $this->Xls_col++;
   }
   //----- sigla_impostos
   function NM_export_sigla_impostos()
   {
         ...
         if ($this->Use_phpspreadsheet) {
             $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->sigla_impostos, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
         }
         else {
             $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->sigla_impostos, PHPExcel_Cell_DataType::TYPE_STRING);
         }
         $this->Xls_col++;
   }
   //----- descricao_imposto
   function NM_export_descricao_imposto()
   {
         ...
         if ($this->Use_phpspreadsheet) {
             $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->descricao_imposto, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
         }
         else {
             $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->descricao_imposto, PHPExcel_Cell_DataType::TYPE_STRING);
         }
         $this->Xls_col++;
   }
   //----- percentual_default
   function NM_export_percentual_default()
   {
         ...
         if (is_numeric($this->percentual_default))
         {
             $this->NM_ctrl_style[$current_cell_ref]['format'] = '#,##0';
         }
         $this->Nm_ActiveSheet->setCellValue($current_cell_ref . $this->Xls_row, $this->percentual_default);
         $this->Xls_col++;
   }
   //----- tipo_imposto
   function NM_export_tipo_imposto()
   {
         ...
         if ($this->Use_phpspreadsheet) {
             $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->tipo_imposto, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
         }
         else {
             $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->tipo_imposto, PHPExcel_Cell_DataType::TYPE_STRING);
         }
         $this->Xls_col++;
   }
   //----- idimpostos
   function NM_sub_cons_idimpostos()
   {
         $this->idimpostos = NM_charset_to_utf8($this->idimpostos);
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $this->idimpostos;
   ...
   //----- sigla_impostos
   function NM_sub_cons_sigla_impostos()
   {
         $this->sigla_impostos = html_entity_decode($this->sigla_impostos, ENT_COMPAT, $_SESSION['scriptcase']['charset']);
         $this->sigla_impostos = strip_tags($this->sigla_impostos);
         $this->sigla_impostos = NM_charset_to_utf8($this->sigla_impostos);
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $this->sigla_impostos;
   ...
   //----- descricao_imposto
   function NM_sub_cons_descricao_imposto()
   {
         $this->descricao_imposto = html_entity_decode($this->descricao_imposto, ENT_COMPAT, $_SESSION['scriptcase']['charset']);
         $this->descricao_imposto = strip_tags($this->descricao_imposto);
         $this->descricao_imposto = NM_charset_to_utf8($this->descricao_imposto);
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $this->descricao_imposto;
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['align']  = "left";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['type']   = "char";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['format'] = "";
         $this->Xls_col++;
   ...
   //----- percentual_default
   function NM_sub_cons_percentual_default()
   {
         $this->percentual_default = NM_charset_to_utf8($this->percentual_default);
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $this->percentual_default;
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['align']  = "right";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['type']   = "num";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['format'] = "#,##0";
         $this->Xls_col++;
   ...
   //----- tipo_imposto
   function NM_sub_cons_tipo_imposto()
   {
         $this->tipo_imposto = html_entity_decode($this->tipo_imposto, ENT_COMPAT, $_SESSION['scriptcase']['charset']);
         $this->tipo_imposto = strip_tags($this->tipo_imposto);
         $this->tipo_imposto = NM_charset_to_utf8($this->tipo_imposto);
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $this->tipo_imposto;
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['align']  = "left";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['type']   = "char";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['format'] = "";
         $this->Xls_col++;
   ...
   {
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_impostos']['nolabel']) || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_impostos']['nolabel'])
       {
   ...
   {
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_impostos']['xls_file']);
      if (is_file($this->Xls_f))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_impostos']['xls_file'] = $this->Xls_f;
      }
...
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_othr_grid_title'] ?> dbo.impostos :: Excel</TITLE>
...
<INPUT type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"> 
<INPUT type="hidden" name="nmgp_opcao" value="<?php echo NM_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dbo_impostos']['xls_return']); ?>"> 
</FORM> 
</BODY>
</HTML>
<?php
