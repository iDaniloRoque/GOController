...
$old_error_handler = set_error_handler("selecionaExportacao_scriptcase_error_handler");

function selecionaExportacao_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['selecionaExportacao']['contr_erro']) && $_SESSION['scriptcase']['selecionaExportacao']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_selecionaExportacao->contr_selecionaExportacao->Apl_com_erro = true;
            if ($inicial_selecionaExportacao->contr_selecionaExportacao->NM_ajax_flag)
            {
                $inicial_selecionaExportacao->contr_selecionaExportacao->NM_ajax_info['errList']['geral_selecionaExportacao'][] = $msg;
                return;
            }
            selecionaExportacao_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function selecionaExportacao_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['selecionaExportacao'])  && '' != $_SESSION['scriptcase']['error_icon']['selecionaExportacao'])  ? $_SESSION['scriptcase']['error_icon']['selecionaExportacao']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['selecionaExportacao']) && '' != $_SESSION['scriptcase']['error_close']['selecionaExportacao']) ? $_SESSION['scriptcase']['error_close']['selecionaExportacao'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['selecionaExportacao']) && '' != $_SESSION['scriptcase']['error_span_title']['selecionaExportacao']) ? $_SESSION['scriptcase']['error_span_title']['selecionaExportacao'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['selecionaExportacao']) && '' != $_SESSION['scriptcase']['error_icon_title']['selecionaExportacao']) ? $_SESSION['scriptcase']['error_icon_title']['selecionaExportacao'] : '';
?>
