  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_form_dbo_observacao_status" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_form_dbo_observacao_status_form();
	}
...
?>
  ...
  function do_ajax_form_dbo_observacao_status_validate_idobservacaostatus_(iNumLinha)
  {
    var nomeCampo_idobservacaostatus_ = "idobservacaostatus_" + iNumLinha;
    var var_idobservacaostatus_ = scAjaxGetFieldHidden(nomeCampo_idobservacaostatus_);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_dbo_observacao_status_validate_idobservacaostatus_(var_idobservacaostatus_, iNumLinha, var_script_case_init, do_ajax_form_dbo_observacao_status_validate_idobservacaostatus__cb);
  } // do_ajax_form_dbo_observacao_status_validate_idobservacaostatus_

  function do_ajax_form_dbo_observacao_status_validate_idobservacaostatus__cb(sResp)
  {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_dbo_observacao_status_validate_idobservacaostatus__cb

  // ---------- Validate descricao_observacao_status_
  function do_ajax_form_dbo_observacao_status_validate_descricao_observacao_status_(iNumLinha)
  {
    var nomeCampo_descricao_observacao_status_ = "descricao_observacao_status_" + iNumLinha;
    var var_descricao_observacao_status_ = scAjaxGetFieldText(nomeCampo_descricao_observacao_status_);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_dbo_observacao_status_validate_descricao_observacao_status_(var_descricao_observacao_status_, iNumLinha, var_script_case_init, do_ajax_form_dbo_observacao_status_validate_descricao_observacao_status__cb);
  } // do_ajax_form_dbo_observacao_status_validate_descricao_observacao_status_

  function do_ajax_form_dbo_observacao_status_validate_descricao_observacao_status__cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    iLineNumber = scAjaxGetLineNumber();
    if ("" != iLineNumber)
    {
      sFieldValid = "descricao_observacao_status_" + iLineNumber;
    }
    else
    {
      sFieldValid = "descricao_observacao_status_";
    }
    scEventControl_onBlur(sFieldValid);
    scAjaxUpdateFieldErrors(sFieldValid, "valid");
    sFieldErrors = scAjaxListFieldErrors(sFieldValid, false);
    if ("" == sFieldErrors)
    {
      var sImgStatus = sc_img_status_ok;
      scAjaxHideErrorDisplay(sFieldValid);
      scErrorLineOff(iLineNumber, "descricao_observacao_status_");
    }
    else
    {
      var sImgStatus = sc_img_status_err;
      scAjaxShowErrorDisplay(sFieldValid, sFieldErrors);
      scErrorLineOn(iLineNumber, "descricao_observacao_status_");
    }
    var $oImg = $('#id_sc_status_' + sFieldValid);
    if (0 < $oImg.length)
    {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_dbo_observacao_status_validate_descricao_observacao_status__cb

  var sc_num_ult_line = "";
  var sc_insert_open  = false;

  // ---------- add_new_line
  function do_ajax_form_dbo_observacao_status_add_new_line(sc_clone, sc_seq_clone)
  {
    if (sc_insert_open)
    {
        if (sc_clone == 'S' && sc_seq_clone != iAjaxNewLine)
        {
          do_ajax_form_dbo_observacao_status_cancel_insert(iAjaxNewLine);
        }
        else
        {
          return;
    ...
    var var_script_case_init = document.F1.script_case_init.value;
    scAjaxProcOn(true);
    x_ajax_form_dbo_observacao_status_add_new_line(var_sc_clone, var_sc_seq_clone, var_sc_seq_vert, var_script_case_init, do_ajax_form_dbo_observacao_status_add_new_line_cb);
  } // do_ajax_form_dbo_observacao_status_add_new_line

  function do_ajax_form_dbo_observacao_status_add_new_line_cb(sResp)
  {
    scAjaxProcOff(true);
    if ("{" == sResp.substr(0, 1)) {
        oResp = scAjaxResponse(sResp);
  ...
  } // do_ajax_form_dbo_observacao_status_add_new_line_cb

  // ---------- backup_line
  function do_ajax_form_dbo_observacao_status_backup_line(iNumLinha)
  {
    var var_idobservacaostatus_ = scAjaxGetFieldHidden("idobservacaostatus_" + iNumLinha);
    var var_nmgp_refresh_row = iNumLinha;
    var var_nm_form_submit = document.F1.nm_form_submit.value;
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_dbo_observacao_status_backup_line(var_idobservacaostatus_, var_nmgp_refresh_row, var_nm_form_submit, var_script_case_init, do_ajax_form_dbo_observacao_status_backup_line_cb);
  } // do_ajax_form_dbo_observacao_status_backup_line

  function do_ajax_form_dbo_observacao_status_backup_line_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    if (!scAjaxHasError())
  ...
  } // do_ajax_form_dbo_observacao_status_backup_line_cb

  function do_ajax_form_dbo_observacao_status_cancel_insert(iSeqVert)
  {
    var oTBodyOld = document.getElementById("hidden_bloco_0").tBodies[0];
  ...
  } // do_ajax_form_dbo_observacao_status_cancel_insert

  function do_ajax_form_dbo_observacao_status_cancel_update(iSeqVert)
  {
    do_ajax_form_dbo_observacao_status_backup_line(iSeqVert);
...
?>
  } // do_ajax_form_dbo_observacao_status_cancel_update

  function do_ajax_form_dbo_observacao_status_restore_buttons()
  {
<?php
...
?>
  } // do_ajax_form_dbo_observacao_status_restore_buttons

  // ---------- table_refresh
  function do_ajax_form_dbo_observacao_status_table_refresh()
  {
    var var_idobservacaostatus_ = document.F2.idobservacaostatus_.value;
    ...
    scAjaxProcOn();
    x_ajax_form_dbo_observacao_status_table_refresh(var_idobservacaostatus_, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_fast_search, var_nmgp_cond_fast_search, var_nmgp_arg_fast_search, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_form_dbo_observacao_status_table_refresh_cb);
  } //  do_ajax_form_dbo_observacao_status_table_refresh

  function do_ajax_form_dbo_observacao_status_table_refresh_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    document.F2.idobservacaostatus_.value = scAjaxGetKeyValue("idobservacaostatus_");
    ...
    scAjaxAlert(do_ajax_form_dbo_observacao_status_table_refresh_cb_after_alert);
  } // do_ajax_form_dbo_observacao_status_table_refresh_cb
  function do_ajax_form_dbo_observacao_status_table_refresh_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    ...
    sc_insert_open = false;
  } // do_ajax_form_dbo_observacao_status_table_refresh_cb_after_alert
...
function scJs_sweetalert_params(params) {
  ...
  var sc_num_ult_opc  = "";
  var sc_num_ult_tr   = "";
  function do_ajax_form_dbo_observacao_status_submit_form(iNumLinha, indexTR)
  {
    if (scEventControl_active(iNumLinha)) {
      setTimeout(function() { do_ajax_form_dbo_observacao_status_submit_form(iNumLinha, indexTR); }, 500);
      return;
    }
    ...
    scAjaxHideMessage();
    var var_idobservacaostatus_ = scAjaxGetFieldHidden("idobservacaostatus_" + iNumLinha);
    var var_descricao_observacao_status_ = scAjaxGetFieldText("descricao_observacao_status_" + iNumLinha);
    var var_nm_form_submit = document.F1.nm_form_submit.value;
...
?>
    x_ajax_form_dbo_observacao_status_submit_form(var_idobservacaostatus_, var_descricao_observacao_status_, var_nmgp_refresh_row, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_form_dbo_observacao_status_submit_form_cb);
  } // do_ajax_form_dbo_observacao_status_submit_form

  function do_ajax_form_dbo_observacao_status_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
      ...
      scAjaxHideErrorDisplay("descricao_observacao_status_" + sc_num_ult_line);
<?php
if (isset($this->Embutida_form) && $this->Embutida_form) {
?>
      scAjaxSetReadonly();
...
?>
    ...
    scAjaxSetLabel(true);
    scAjaxSetMaster();
    scAjaxAlert(do_ajax_form_dbo_observacao_status_submit_form_cb_after_alert);
  } // do_ajax_form_dbo_observacao_status_submit_form_cb
  function do_ajax_form_dbo_observacao_status_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_form_dbo_observacao_status_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_form_dbo_observacao_status_navigate_form()
  {
    if (scFormHasChanged) {
      scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', perform_ajax_form_dbo_observacao_status_navigate_form, function() {});
    } else {
      perform_ajax_form_dbo_observacao_status_navigate_form();
    }
  }

  function perform_ajax_form_dbo_observacao_status_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    ...
    if (sc_insert_open)
    {
        do_ajax_form_dbo_observacao_status_cancel_insert(sc_num_ult_line);
    }
    nm_uncheck_delete();
    scAjaxHideMessage();
    scAjaxHideErrorDisplay("table");
    for (iNavForm = 1; iNavForm < <?php echo $this->sc_max_reg; ?> + 1; iNavForm++)
    {
      ...
      scAjaxHideErrorDisplay("descricao_observacao_status_" + iNavForm);
    }
    var var_idobservacaostatus_ = document.F2.idobservacaostatus_.value;
    var var_nm_form_submit = document.F2.nm_form_submit.value;
    var var_nmgp_opcao = document.F2.nmgp_opcao.value;
    ...
    var var_script_case_init = document.F2.script_case_init.value;
    scAjaxProcOn();
    x_ajax_form_dbo_observacao_status_navigate_form(var_idobservacaostatus_, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_form_dbo_observacao_status_navigate_form_cb);
  } // do_ajax_form_dbo_observacao_status_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_dbo_observacao_status']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_form_dbo_observacao_status_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetFields(false);
    scAjaxSetVariables();
    document.F2.idobservacaostatus_.value = scAjaxGetKeyValue("idobservacaostatus_" + var_last_index);
    ...
    scErrorLineReset();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_form_dbo_observacao_status_navigate_form_cb_after_alert);
  } // do_ajax_form_dbo_observacao_status_navigate_form_cb
  function do_ajax_form_dbo_observacao_status_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
<?php
...
?>
  ...
  } // do_ajax_form_dbo_observacao_status_navigate_form_cb_after_alert
  function sc_hide_form_dbo_observacao_status_form()
  {
...
?>
  function ajax_create_tables(iNumLinha)
  {
    ...
    iTotCampos++;
    ajax_field_list[iTotCampos] = "descricao_observacao_status_" + iNumLinha;
    iTotCampos++;
    ajax_error_list["idobservacaostatus_" + iNumLinha] = {"label": "Código<?php echo $sLineInfo; ?>", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5};
    ajax_error_list["descricao_observacao_status_" + iNumLinha] = {"label": "Descrição<?php echo $sLineInfo; ?>", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5};
    ajax_field_mult["idobservacaostatus_"][iNumLinha] = "idobservacaostatus_" + iNumLinha;
    ajax_field_mult["descricao_observacao_status_"][iNumLinha] = "descricao_observacao_status_" + iNumLinha;
    ...
    ajax_field_id["descricao_observacao_status_" + iNumLinha] = new Array("hidden_field_label_descricao_observacao_status_", "hidden_field_data_descricao_observacao_status_" + iNumLinha);
    ajax_error_count["idobservacaostatus_" + iNumLinha] = "off";
    ajax_error_count["descricao_observacao_status_" + iNumLinha] = "off";
<?php
if (!$this->Grid_editavel)
{
?>
    ajax_read_only["idobservacaostatus_" + iNumLinha] = "on";
    ajax_read_only["descricao_observacao_status_" + iNumLinha] = "off";
<?php
}
else
{
?>
    ajax_read_only["idobservacaostatus_" + iNumLinha] = "on";
    ajax_read_only["descricao_observacao_status_" + iNumLinha] = "on";
<?php
}
?>
  }
  function ajax_destroy_tables(iNumLinha)
  {
    ...
    ajax_error_list["descricao_observacao_status_" + iNumLinha] = null;
  }
  ...
  var ajax_field_mult = {
    ...
    "descricao_observacao_status_": new Array()
  };
  ...
  function scRemoveErrors()
  {
    for (iNumLinha = 1; iNumLinha < Lim_linhas; iNumLinha++)
    {
      ...
      ajax_error_list["descricao_observacao_status_" + iNumLinha]["valid"] = new Array();
      ajax_error_list["descricao_observacao_status_" + iNumLinha]["onblur"] = new Array();
      ajax_error_list["descricao_observacao_status_" + iNumLinha]["onchange"] = new Array();
      ajax_error_list["descricao_observacao_status_" + iNumLinha]["onclick"] = new Array();
      ajax_error_list["descricao_observacao_status_" + iNumLinha]["onfocus"] = new Array();
    }
  }
...
?>
  ...
  {
<?php
  $NM_contr_readonly = (isset($this->nmgp_cmp_readonly['idobservacaostatus_'])) ? $this->nmgp_cmp_readonly['idobservacaostatus_'] : 'on';
?>
    scAjaxFieldRead("idobservacaostatus_" + iSeq, "<?php echo $NM_contr_readonly ?>");
<?php
  $NM_contr_readonly = (isset($this->nmgp_cmp_readonly['descricao_observacao_status_'])) ? $this->nmgp_cmp_readonly['descricao_observacao_status_'] : 'off';
?>
    scAjaxFieldRead("descricao_observacao_status_" + iSeq, "<?php echo $NM_contr_readonly ?>");
  }

  function mdCloseObjects(iSeq)
  {
    scAjaxFieldRead("idobservacaostatus_" + iSeq, "on");
    scAjaxFieldRead("descricao_observacao_status_" + iSeq, "on");
    rerunHeaderDisplay = 1;
    scSetFixedHeaders(true);
  }
...
?>
  ...
  function scErrorLineOn(iRow, sIdError)
  {
    ...
    if (bErrorRow || ("on" == ajax_error_count["idobservacaostatus_" + iRow] || "on" == ajax_error_count["descricao_observacao_status_" + iRow]))
    {
      ...
      $("#hidden_field_data_descricao_observacao_status_" + iRow).addClass("scFormErrorLine");
    }
  }
  ...
  function scErrorLineOff(iRow, sIdError)
  {
    ...
    if (bErrorRow || ("off" == ajax_error_count["idobservacaostatus_" + iRow] && "off" == ajax_error_count["descricao_observacao_status_" + iRow]))
    {
      if (bErrorRow)
      {
        ajax_error_count["idobservacaostatus_" + iRow] = "off";
        ajax_error_count["descricao_observacao_status_" + iRow] = "off";
      }
      ...
      $("#hidden_field_data_descricao_observacao_status_" + iRow).removeClass("scFormErrorLine");
    }
  }
  ...
  function scAjaxDetailValue(sIndex, sValue)
  {
    ...
    if ("descricao_observacao_status_" == sIndex)
    {
