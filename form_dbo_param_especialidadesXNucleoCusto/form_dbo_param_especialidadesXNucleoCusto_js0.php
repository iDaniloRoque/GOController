<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="idparam_especialidade_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['idparam_especialidade_']); ?>">
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_dbo_param_especialidadesXNucleoCusto"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
function sc_btn_ListaNucleos()
{
    if (scEventControl_active_all()) {
      setTimeout(function() { sc_btn_ListaNucleos(); }, 500);
      return;
    }
    sc_btn_ListaNucleos_ok();
}
function sc_btn_ListaNucleos_cancel()
{
}
function sc_btn_ListaNucleos_ok()
{
  tb_show('', '<?php echo $this->Ini->link_grid_dbo_Nucleo_de_atendimento_cons; ?>?script_case_init=<?php echo $this->Ini->sc_page; ?>&nmgp_url_saida=modal&nmgp_parms=<?php echo ""; ?>&nmgp_outra_jan=true&TB_iframe=true&height=650&width=840&modal=true', '');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_dbo_param_especialidadesXNucleoCusto']['where_detal']))
  {
?>  
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_form_dbo_param_especialidadesXNucleoCusto_submit_form(y, z);
