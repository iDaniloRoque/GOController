...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['pre_lancamento_controle']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="pre_lancamento_controle"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
function sc_btn_Cadastrar()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_Cadastrar(); }, 500);
      return;
    }
    sc_btn_Cadastrar_ok();
}
function sc_btn_Cadastrar_cancel()
{
}
function sc_btn_Cadastrar_ok()
{
  tb_show('', '<?php echo $this->Ini->link_form_dbo_usuario_externo_edit; ?>?script_case_init=<?php echo $this->Ini->sc_page; ?>&nmgp_url_saida=modal&nmgp_parms=<?php echo "NM_btn_insert*scinS*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutsc_redir_atualiz*scinok*scoutsc_redir_insert*scinok*scout"; ?>&nmgp_outra_jan=true&TB_iframe=true&height=450&width=400&modal=true', '');
}
var Crtl_btn_Lancar = false;
function sc_btn_Lancar()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_Lancar(); }, 500);
      return;
    }
    if (Crtl_btn_Lancar) {return;}
    sc_btn_Lancar_ok();
}
function sc_btn_Lancar_cancel()
{
}
function sc_btn_Lancar_ok()
{
    Crtl_btn_Lancar = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?Lancar?@?";
    document.F1.action = "./";
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['pre_lancamento_controle']['where_detal']))
  {
?>  
...
{
  if (scFormHasChanged) {
    scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', function() { document.F4.target = "_self"; document.F4.action = "pre_lancamento_controle_fim.php"; document.F4.submit(); }, function() {});
  } else {
    document.F4.target = "_self";
    document.F4.action = "pre_lancamento_controle_fim.php";
    document.F4.submit();
  }
}
