...
$old_error_handler = set_error_handler("filtro_recibos_scriptcase_error_handler");

function filtro_recibos_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['filtro_recibos']['contr_erro']) && $_SESSION['scriptcase']['filtro_recibos']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_filtro_recibos->contr_filtro_recibos->Apl_com_erro = true;
            if ($inicial_filtro_recibos->contr_filtro_recibos->NM_ajax_flag)
            {
                $inicial_filtro_recibos->contr_filtro_recibos->NM_ajax_info['errList']['geral_filtro_recibos'][] = $msg;
                return;
            }
            filtro_recibos_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function filtro_recibos_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['filtro_recibos'])  && '' != $_SESSION['scriptcase']['error_icon']['filtro_recibos'])  ? $_SESSION['scriptcase']['error_icon']['filtro_recibos']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['filtro_recibos']) && '' != $_SESSION['scriptcase']['error_close']['filtro_recibos']) ? $_SESSION['scriptcase']['error_close']['filtro_recibos'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['filtro_recibos']) && '' != $_SESSION['scriptcase']['error_span_title']['filtro_recibos']) ? $_SESSION['scriptcase']['error_span_title']['filtro_recibos'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['filtro_recibos']) && '' != $_SESSION['scriptcase']['error_icon_title']['filtro_recibos']) ? $_SESSION['scriptcase']['error_icon_title']['filtro_recibos'] : '';
?>
